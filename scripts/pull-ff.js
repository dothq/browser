...
const main = async () => {
    if (existsSync(resolve(process.cwd(), "core"))) {
        ...
        if (platform() == "win32") {
            if (!existsSync(resolve("C:", "mozilla-build", "start-shell.bat"))) {
                console.error(`Mozillabuild is not installed!`);
                console.error(``);
                console.error(`In order to build on a Windows machine, it is required`)
                console.error(`that any commands relating to the engine should be executed`)
                console.error(`within the Mozillabuild shell to reduce incompabilities.`)
                console.error(``);
                console.error(`Please install it from https://ftp.mozilla.org/pub/mozilla/libraries/win32/MozillaBuildSetup-Latest.exe,`);
                console.error(`and re-run this command from the Mozillabuild shell at ${resolve("C:", "mozilla-build", "start-shell.bat")}`)
        ...
        await sh("python3", [bootstrapPath, "--vcs=git", "--no-interactive"], { stdio: "inherit" });
    }
}
