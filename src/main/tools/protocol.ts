...
export const startProtocolService = () => {
    ...
    if(process.env.ENV == "development") {
        ses.protocol.registerBufferProtocol('dot', (request, callback) => {
            ...
            callback({ mimeType: 'text/html', data: Buffer.from(`<script>document.title = "${parsed.host.substr(0, 1).toUpperCase() + parsed.host.substr(1, parsed.host.length)}"</script><iframe src="http://localhost:9015/${parsed.path == "/" ? parsed.host + ".html" : parsed.path.split("/")[1] + `?t=${Date.now()}`}" style="position:fixed; top:0; left:0; bottom:0; right:0; width:100%; height:100%; border:none; margin:0; padding:0; overflow:hidden; z-index:999999;">`) })
        }, (error) => {
