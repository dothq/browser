...
export const Addressbar = observer(({ WEBUI_PREFIX, NEWTAB_URL, WEBUI_SUFFIX, dot }: { WEBUI_PREFIX: string;  NEWTAB_URL: string; WEBUI_SUFFIX?: string; dot: any }) => {
    ...
    const searchWidth = url && url.startsWith(WEBUI_PREFIX) ? 118 : url && isIdle ? isError ? 0 : url.startsWith("http://") ? 114 : 0 : 0;
    const showSearchText = url && url.startsWith(WEBUI_PREFIX) || url && isIdle ? isError ? false : url.startsWith("http://") : false;

    return (
        <StyledAddressbar>
            <SearchIcon WEBUI_PREFIX={WEBUI_PREFIX} WEBUI_SUFFIX={WEBUI_SUFFIX} NEWTAB_URL={NEWTAB_URL} dot={dot}/>
            <Input 
                ref={dot.searchRef} 
...
const SearchIcon = observer(({ WEBUI_PREFIX, NEWTAB_URL, WEBUI_SUFFIX, dot }: { WEBUI_PREFIX: string; NEWTAB_URL: string; WEBUI_SUFFIX?: string; dot: any }) => {
    ...
    if(dot.tabs.selectedTab) {
        ...
        // if(tab && tab.url.startsWith(WEBUI_PREFIX)) icon = "circle"
