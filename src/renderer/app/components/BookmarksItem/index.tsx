...
export const BookmarksItem = observer(({ url, title, favicon }: { url: string; title?: string; favicon: string }) => {
    ...
    const onBookmarksItemMouseUp = (e, url) => {
        ...
        if(e.button == 2) return ipcRenderer.send('bookmark-menu-popup', { bookmark: { url, title, favicon }, x: e.clientX, y: e.clientY })
    }
